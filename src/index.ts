...
client.once(Events.ClientReady, async (readyClient) => {
    ...
    try {
        ...
        // Initialize auto-update system
        autoUpdateSystem = new AutoUpdateSystem(client, db);
...
async function handleKagamiCommand(message: Message, args: string[]) {
...
// Whoami command
async function handleWhoamiCommand(message: Message) {
...
// Commands list
async function handleCmdsCommand(message: Message) {
    const commandsEmbed = {
...
async function handleFactCheckCommand(message: Message, args: string[]) {
    if (args.length === 0) {
...
function startUpdateCheckCronJob() {
    ...
    cron.schedule(cronPattern, async () => {
        ...
        try {
            const autoUpdater = new AutoUpdateSystem(client, db);
