...
class ServerCommands {
    ...
    async handleServerCommand(message, args) {
    ...
    async toggleMaintenance(message, args) {
        if (!(0, admin_1.requireAdmin)(message))
            return;
    ...
    async pauseOperations(message, args) {
        if (!(0, admin_1.requireAdmin)(message))
            return;
    ...
    async resumeOperations(message) {
        if (!(0, admin_1.requireAdmin)(message))
            return;
        this.isMaintenanceMode = false;
    ...
    async sendMaintenanceNotification(message, action) {
        ...
        await channel.send(messages[action] || 'KagamiMe is performing maintenance...');
    }
    isInMaintenanceMode() {
        return this.isMaintenanceMode;
    }
    async showServerHelp(message) {
        if (!(0, admin_1.requireAdmin)(message))
            return;
        const embed = {
